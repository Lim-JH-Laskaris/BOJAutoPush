def int_discount(want, discount):
    # want의 원소를 기준으로 discount의 원소를 정수화, 없는 변수는 모두 10로
    want_dict = {want[i]:i for i in range(len(want))}
    return [want_dict[j] if j in want else 10 for j in discount]

def number_to_len11(number):
    # number를 길이가 11인 리스트로 변환 (나머지는 모두 0)
    return number+[0]*(11-len(number))
...
def solution(want, number, discount):
    ...
    discount = int_discount(want, discount)
    # number를 길이가 11인 리스트로 변환 (나머지는 모두 0)

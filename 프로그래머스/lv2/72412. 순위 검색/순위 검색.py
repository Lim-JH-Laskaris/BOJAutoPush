...
def binary_compare_count(sorted_list, number):
    """이진 탐색 방식으로, 정렬된 리스트에서 특정 숫자보다 큰 숫자의 개수를 찾는 함수"""
    return len(sorted_list) - bisect.bisect_left(sorted_list, number)
...
def solution(info, query):
    ...
    list_query = list(map(split_and_int,map(lambda x : x.replace(' and ',' '),query))) 
